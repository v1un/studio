...
const QuestSchemaInternal = z.object({
  id: z.string().describe("A unique identifier for the quest, e.g., 'quest_main_chapter2_001'. Must be unique among all quests being generated for this chapter. REQUIRED."),
  title: z.string().optional().describe("A short, engaging title for the quest, fitting the chapter and series."),
  description: z.string().describe("A clear description of the quest's overall objective, fitting the series and chapter context. REQUIRED."),
  ...
  chapterId: z.string().describe("The ID of the chapter this quest belongs to (must match the input chapterToFleshOut.id). REQUIRED."),
...
const fleshOutChapterQuestsFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const prompt = ai.definePrompt({
      ...
      config: modelConfig,
      prompt: `IMPORTANT_INSTRUCTION: Your entire response MUST be a single, valid JSON object conforming to the 'FleshOutChapterQuestsOutputSchema'. The 'fleshedOutQuests' array is REQUIRED. Each quest object within it MUST have 'id', 'description', 'type', 'status', and 'chapterId' fields. If 'objectives' are included, each must have 'description' and 'isCompleted'. If 'items' are included in rewards, each item MUST have 'id', 'name', and 'description'.
...
Ensure all field names and values in your JSON response strictly match the types and requirements described in the FleshOutChapterQuestsOutputSchema definition provided earlier in this prompt.`,
    ...
    if (!output || !output.fleshedOutQuests) {
      ...
      throw new Error('AI failed to generate quests for the chapter (REQUIRED "fleshedOutQuests" field missing).');
    }
    ...
    const validatedQuests = output.fleshedOutQuests.map((q, index) => ({
        ...
        description: q.description || "No description provided by AI.", // Ensure description exists
